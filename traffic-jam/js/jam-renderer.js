        ...
        for (var x = 0; x < this.jam.width; x++) {
            for (var y = 0; y < this.jam.height; y++) {
                var polygon = new fabric.Rect({
                    ...
                    angle: 0,
                    fill: this.colors[this.jam.grid[x][y]],
                    strokeLineJoin: 'round',
                    stroke: 'black',
                    strokeWidth: 1,
                    hasControls: false,
...
export function render_jams(id, jams, renderX = 120, renderY = 0, renderW = 100, renderH = 100) {
    var canvas = new fabric.Canvas(id);
    var renderers = [];

    for (var i = 0; i < jams.length; i++) {
        renderers.push(new JamRenderer(jams[i], canvas, i * renderX, renderY, renderW, renderH));
    }
}